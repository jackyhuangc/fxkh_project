    ...
    <table class="tablelist">
      ...
      <tbody>
        <tr v-for="(item,index) in listGrid">
          ...
          <td>{{(item.ttst/100).toFixed(2)}}</td>
          <td>{{(item.incp/10).toFixed(2)}}</td>
          <td>{{item.adunnm}}</td>
          <td>
            <a href="#" class="tablelink" @click="yuan_click(index,item)">预案浏览</a>
            <!--&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="tablelink" @click="modify_click(item.ennmcd)">详细</a>-->
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
    ...
    return {
      ...
      drainages: {
        ...
        data: [
          ...
          { value: '长江干流', text: '　├长江干流水系' },
          { value: '金沙江', text: '　├金沙江水系' },
          { value: '岷江', text: '　├岷江水系' },
          { value: '大渡河', text: '　├大渡河水系' },
          { value: '嘉陵江', text: '　├嘉陵江水系' },
          { value: '渠江', text: '　├渠江水系' },
  ...
  methods: {
        ...
        .then((response) => {
          ...
          if (response.data.code = '000') {
            ...
            if (page == 1) {
              ...
              this.$http.get("/api/PlatResource/QueryRSProjectCount"
                + "?ennm=" + this.ennm
                + "&dscode=" + (this.countries.selected != '000000' ? this.countries.selected : this.states.selected)
                + "&drainage=" + this.drainages.selected
                + "&encl=" + this.encls.selected, {
    ...
    yuan_click: function (index, item) {
      // 预案浏览
    ...
    modify_click: function (ennmcd) {
      ...
      //     }
      // });  
      window.open('http://10.51.101.206:8100/sc/waterproject/rs/' + ennmcd + '/detail/?ref=/waterproject/rs/', '_blank');
    },
