...
export default {
        ...
        .then(response => {
          if ((response.data.code = "000")) {
            ...
            for (var i = 0; i < response.data.data.length; i++) {
              array.push({
                value: response.data.data[i].districtCode,
                text: response.data.data[i].districtName
              });
            }
            console.log(array);
          ...
          } else layer.msg(response.data.error);
        })
  ...
  methods: {
    ...
    loadIpResources(data) {
      if (!data) {
        data = [{ name: "海门", value: 90 }];
      }

      var geoCoordMap = {
        "San Francisco": [-122.445363, 37.626557],
        Sydney: [151.150352, -33.897539],
        Singapore: [103.883097, 1.361633],
        台湾: [121.55945, 25.049137],
        海门: [121.15, 31.89],
    ...
    initMap() {
      var option = {
        ...
        tooltip: {
          ...
          //   //     console.log(myseries[i].data[j].name);
