...
<template>
	<div id="app">
		<div class="tools">
			<ul class="toolbar" style="margin-top:8px;">
				<li>
					...
					<input class="datainp" id="docname" type="text" style="width:377px;" placeholder="" value="" v-model="docname"   @keyup="show($event)"/>
				</li>
				<li>
      ...
      <li>
				...
				<li>
					<a href="javascript:void(0);" @click="upLoad()"><i class="fa fa-plus"></i>上传预案</a>
				</li>
			</ul>
		</div>
		...
		<table class="tablelist">
			...
			<tbody>
				<tr v-for="(emp,index) in listResult">
					...
					<td><a href="#" class="tablelink" @click="viewDoc(index)">预览</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="#" class="tablelink" @click="deleteDoc(index)"> 删除</a></td>
				</tr>  
			</tbody>
		</table>
      ...
      <div style="float:left;"><div style="padding-left:17px; margin-top:-2px;font-size:14px;">{{total}}</div></div>
  </div>
  </div>
</template>
<script>
 ...
 data(){
  ...
  methods: {
    query: function (page) {
      ...
      //layer.msg("预案查询...");
      this.$http.get("http://localhost:28025/api/PlatResource/QueryEmergencyPlan"
      +"?page=" + page
      ...
      +"&planlevel="+this.planlevels.selected
        ...
        .then((response) => {
          if (response.data.code = '000') {
            ...
            if (page == 1) {
              // 获取总页数
              this.$http.get("http://localhost:28025/api/PlatResource/QueryEmergencyPlanCount"   
              +"?docname="+this.docname
              +"&districtcode="+(this.countries.selected!='000000'?this.countries.selected:this.states.selected)
              +"&plantype="+this.plantypes.selected
              +"&planlevel="+this.planlevels.selected
    ...
    viewDoc: function (index) {
        ...
        this.$http.get("http://localhost:28025/api/PlatResource/GetDocument?docid=" + this.listResult[index].planDocID)
        .then((response) => {
    ...
    deleteDoc: function (index) {
      ...
      //layer.msg(this.listResult[index]);
        ...
        this.$http.post("http://localhost:28025/api/PlatResource/DeleteEmergencyPlan",
          {
